...
function renderCalendar() {
    ...
    for (let i = 0; i < 7; i++) {
        if (i < firstDay) tr += "<td></td>";
            ...
            tr += createDayCell(year, month, dayCounter);
            dayCounter++;
        }
    }
    tr += "</tr>";
    ...
    while (dayCounter <= daysInMonth) {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            else tr += createDayCell(year, month, dayCounter);
            dayCounter++;
        }
        tr += "</tr>";
        calendar.innerHTML += tr;
...
function createDayCell(year, month, day) {
    ...
    return `<td class="day-cell" data-date="${dateStr}">
...
function showEvents(dayEvents, date) {
    ...
    if (dayEvents.length === 0) {
        ...
        dayEvents.forEach(e => {
            eventList.innerHTML += `<li class="${e.category}">${e.title}</li>`;
        });
    }
}
...
function highlightToday() {
    ...
    document.querySelectorAll(".day-cell").forEach(cell => {
        if (cell.dataset.date === today) cell.classList.add("today");
    });
}
...
function updateNextHoliday() {
    ...
    if (upcoming.length === 0) {
        holidayCountdown.textContent = "Ближайших праздников нет";
        return;
    }
...
function highlightNextHoliday() {
    ...
    const next = upcoming[0];
    document.querySelectorAll(".day-cell").forEach(cell => {
        if (cell.dataset.date === next.date) cell.classList.add("next-holiday");
    });
}
